...
use cxtvault_mcp::transport::http::SingleCorpusServerState;
...
async fn test_mcp_http_server_and_client_e2e() {
    ...
    assert_eq!(init["serverInfo"]["name"], "cxtvault");
...
async fn handle_jsonrpc_test(
    ...
    axum::Json(req): axum::Json<cxtvault_mcp::transport::JsonRpcRequest>,
) -> axum::response::Response {
    use axum::response::IntoResponse;
    let mut engine = state.engine.lock().await;
    let res = cxtvault_mcp::transport::dispatch::dispatch(&req, &mut *engine, &state.registry);
    if let Some(id) = req.id {
        let rpc_res = cxtvault_mcp::transport::dispatch::format_rpc_response(id, res);
