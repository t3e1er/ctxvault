...
use cxtvault_common::config::{EdgeClass, EdgeDirection, EdgeSource, EdgeTypeConfig};
use cxtvault_common::types::{Document, EdgeProvenance};
...
impl KnowledgeGraph {
    ...
    pub fn extract_lineage_for_node(
        &self,
        path: &str,
    ) -> Option<cxtvault_common::types::LineageAnnotation> {
        let &idx = self.node_map.get(path)?;

        let mut ann = cxtvault_common::types::LineageAnnotation::default();
...
mod tests {
    use super::*;
    use cxtvault_common::config::{EdgeClass, EdgeSource, EdgeTypeConfig};
    use cxtvault_common::types::{Document, WikiLink};
