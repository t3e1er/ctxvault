...
use cxtvault_common::config::CorpusConfig;
use cxtvault_common::types::Document;
...
impl Engine {
    ...
    pub fn full_reindex(&mut self) -> Result<usize> {
        ...
        let tag_configs: Vec<_> = self
            ...
            .edge_types
            .iter()
            .filter(|et| et.source == cxtvault_common::config::EdgeSource::Tag)
            .cloned()
            .collect();
    ...
    pub fn reembed(&mut self) -> Result<usize> {
        ...
        for file in &files {
            ...
            let parsed_chunks: Option<(Vec<cxtvault_common::types::Chunk>, Option<String>)> =
            ...
            let (chunks, title) = if let Some((c, t)) = parsed_chunks {
                ...
                let chunks: Vec<cxtvault_common::types::Chunk> = chunk_records
                    .into_iter()
                    .map(|cr| cxtvault_common::types::Chunk {
...
mod tests {
    ...
    fn test_config(corpus_path: &Path) -> CorpusConfig {
        CorpusConfig {
            ...
            mode: cxtvault_common::config::CorpusMode::ReadWrite,
            chunking: cxtvault_common::config::ChunkingConfig {
                min_chunk_tokens: 1, // very low for tests
                ..Default::default()
            },
            embedding: cxtvault_common::config::EmbeddingConfig::default(),
            graph: cxtvault_common::config::GraphConfig {
                edge_types: vec![cxtvault_common::config::EdgeTypeConfig {
                    name: "Wikilink".to_string(),
                    source: cxtvault_common::config::EdgeSource::Wikilink,
                    weight: 1.0,
                    bidirectional: false,
                    field: None,
                    direction: None,
