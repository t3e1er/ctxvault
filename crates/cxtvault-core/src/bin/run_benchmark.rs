...
use cxtvault_common::config::{CorpusConfig, EdgeClass};
use cxtvault_common::types::SearchResult;
...
fn main() {
    println!("============================================================");
    println!(" CXTVAULT RETRIEVAL QUALITY BENCHMARK RUNNER");
    println!("============================================================");
    ...
    for q in &queries {
        ...
        let sem_res = search::search_semantic_dual(
            ...
            &q.query,
            10,
            cxtvault_common::types::SearchDepth::Precise,
        )
        .unwrap_or_default();
    ...
    report.push_str("# Cxtvault Retrieval Quality Benchmark Report (v2)\n\n");
