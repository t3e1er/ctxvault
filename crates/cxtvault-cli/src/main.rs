...
use cxtvault_mcp::client::McpClient;
use cxtvault_mcp::tools::ToolRegistry;
use cxtvault_mcp::transport;

/// Enterprise semantic MCP server for markdown knowledge bases.
#[derive(Parser, Debug)]
#[command(name = "cxtvault", version, about)]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    ...
    // -----------------------------------------------------------------------
    tracing::info!(mode = ?cli.mode, "starting cxtvault engine");
            ...
            graph: cxtvault_common::config::GraphConfig {
                edge_types: vec![
                    ...
                    cxtvault_common::config::EdgeTypeConfig {
                        ...
                        source: cxtvault_common::config::EdgeSource::Tag,
                        weight: 0.5,
                        bidirectional: true,
                        field: None,
                        direction: None,
