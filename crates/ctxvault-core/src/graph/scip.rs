...
mod tests {
    ...
    fn test_scip_index_ingestion() {
        ...
        occ_def.symbol = "scip-rust cargo ctxvault-core 0.0.22 search().".to_string();
        ...
        occ_ref.symbol = "scip-rust cargo ctxvault-core 0.0.22 rrf_fuse().".to_string();
        ...
        assert_eq!(def_edge.target, "scip-rust cargo ctxvault-core 0.0.22 search().");
        assert_eq!(def_edge.confidence, Some(ResolutionConfidence::High));

        // Check calls edge
        let call_edge = edges.iter().find(|e| e.edge_type == "calls").unwrap();
        assert_eq!(call_edge.source, "scip-rust cargo ctxvault-core 0.0.22 search().");
        assert_eq!(call_edge.target, "scip-rust cargo ctxvault-core 0.0.22 rrf_fuse().");
