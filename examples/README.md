# `cxtvault` Examples & Starter Pack

This directory contains turnkey examples, agent steering configurations, workflow skills, multi-agent swarm blueprints, and a pre-configured starter knowledge vault for `cxtvault`.
...
|---|---|---|
| [`steering/`](steering/) | System prompts and rules for AI assistants (`.cursorrules`, Claude Desktop, Windsurf, Antigravity, generic LLMs) | Drop into your editor or AI configuration to immediately teach your agent how to use `cxtvault` tools efficiently. |
...
## 1. Quick Start: Pointing `cxtvault` at the Starter Vault

You can start `cxtvault` against the included starter vault immediately:

```bash
# From repository root
cxtvault --corpus examples/starter-vault --sync
```

Or test a hybrid query directly using CLI client mode:

```bash
cxtvault --mode client --call search_hybrid --query "How does hybrid retrieval work?"
```
...
Copy [`steering/cxtvault-rules.md`](steering/cxtvault-rules.md) to `.agents/rules/cxtvault-rules.md`, and copy the skill folders in [`skills/`](skills/) to `.agents/skills/`.
